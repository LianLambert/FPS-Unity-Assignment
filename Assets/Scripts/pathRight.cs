...
public class pathRight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (bullet.targetDestroyed)
        {
            ...
            // while the path does not reach to the other end of the canyon
            while (transform.localPosition.x < 22.5f)
            {
                // if not already at right canyon wall
                if (transform.localPosition.z > -22.5f)
                {
                    // move right or straight randomly and make stepping stone
                ...
                // if at right canyon wall, can only go straight
                else
                {
                    transform.Translate(10, 0, 0);
                    Instantiate(pathStone, transform.position, Quaternion.identity, pathStones.transform);
